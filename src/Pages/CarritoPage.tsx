...
const CarritoPage = () => {
    ...
    const enviarWhatsApp = () => {
        ...
        mensaje += `--------------------------%0A`;
        mensaje += `*Total a pagar: ${formatarMoeda(totalPrecio, 'PYG')}*%0A%0A`;
    ...
    // 2. Manejo de disminución de cantidad
    const handleDisminuir = (item: any) => {
        if (item.cantidad > 1) {
            // Nota: Aquí deberías usar una función decreaseQuantity de tu Context si existe
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Lista de Productos (Fondo Blanco, bordes suaves) */}
                <div className="lg:col-span-2 space-y-4">
                    {cart.map((item) => (
                        <div key={item.id} className="bg-white p-4 rounded-[2rem] border border-violet-50 flex flex-col sm:flex-row items-center gap-4 shadow-sm">
                            ...
                            {/* Controles de Cantidad (Estilo Violeta Claro) */}
                ...
                {/* Resumen de Pago (Fondo Violeta Oscuro original) */}
                <div className="lg:col-span-1">
                    <div className="bg-violet-950 text-white p-8 rounded-[2.5rem] shadow-xl sticky top-24">
                        ...
                        <div className="flex justify-between items-end mb-8">
                            <span className="text-violet-300">Total a pagar</span>
                            <span className="text-3xl font-black">{formatarMoeda(totalPrecio, 'PYG')}</span>
                        </div>

                        {/* Botón Principal (Violeta original) */}
                        <button 
                            onClick={enviarWhatsApp}
                            className="w-full bg-violet-600 hover:bg-violet-500 text-white font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-lg shadow-black/20"
                        >
                        ...
                        <button 
                            onClick={clearCart}
                            className="w-full mt-4 text-violet-400 text-sm hover:text-white transition-colors"
                        >
                            Vaciar Carrito
                        </button>
                    </div>
